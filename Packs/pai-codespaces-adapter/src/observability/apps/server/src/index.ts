...
function broadcastNotification(notification: NotificationEvent) {
...
const server = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    if (url.pathname === '/notification' && req.method === 'POST') {
      try {
        const notification: NotificationEvent = await req.json();

        // Validate required fields
        if (!notification.agent_name || !notification.message) {
        ...
        console.log(`📢 Notification from ${notification.agent_name}: ${notification.message.substring(0, 50)}...`);

        return new Response(JSON.stringify({ status: 'ok', broadcast: true }), {
          headers: { ...headers, 'Content-Type': 'application/json' }
        });
      } catch (error) {
